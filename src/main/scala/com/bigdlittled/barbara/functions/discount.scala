...
object discount extends UFunc with discountLowPrio with VectorizedReduceUFunc {
  ...
  implicit def reduce[T, @expand.args(Int, Double, Float, Long) S](implicit iter: CanTraverseValues[T, S]): Impl[T, S] = new Impl[T, S] {
    def apply(v: T): S = {
      class discountVisitor extends ValuesVisitor[S] {
        var discount : S = 0
        def visit(a: S): Unit = {
          discount += a
        }
        ...
        def zeros(numZero: Int, zeroValue: S): Unit = {
          discount += numZero * zeroValue
        }
      }
      val visit = new discountVisitor
  ...
  implicit def reduceSemiring[T, S](implicit iter: CanTraverseValues[T, S], semiring: Semiring[S]): Impl[T, S] = new Impl[T, S] {
    def apply(v: T): S = {
      class discountVisitor extends ValuesVisitor[S] {
        var discount : S = semiring.zero
        def visit(a: S): Unit = {
          discount = semiring.+(discount, a)
        }
...
sealed trait discountLowPrio { this: discount.type =>
